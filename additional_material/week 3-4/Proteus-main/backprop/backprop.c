...
void bpnn_layerforward(float *l1, float *l2, float **conn, int n1, int n2)
{
  ...
  #ifdef RUN_BASELINE
    stopTimer(&timer);
    incrementElapsedTime(timer);
  #endif 

  #ifdef RUN_PIM
    int *conn_j = (int *) malloc(sizeof(int) * (n1 + 1));
    int *output_mult = (int *) malloc(sizeof(int) * (n1 + 1));

    // convert l1 to fixed point
    int *l1_fp = (int *) malloc(sizeof(int) * (n1 + 1));
    for (j = 0; j <= n1; j++) {
      l1_fp[j] = float2fix(l1[j]);
    }

    /*** For each unit in second layer ***/
    for (j = 1; j <= n2; j++) {
      // allocation an array with n1 elements
      for (k = 0; k <= n1; k++) {
        // printf("conn[%d][%d] = %f\n", k, j, conn[k][j]);
        conn_j[k] = float2fix(conn[k][j]);
      }

      /*** Compute weighted sum of its inputs ***/
      sum = 0.0;
      bbop_op(BBOP_MUL, conn_j, l1_fp, output_mult, n1 + 1, 0, BBOP_MUL);
      sum = bbop_op_red(BBOP_ADD, output_mult, n1 + 1, 1);
    }
