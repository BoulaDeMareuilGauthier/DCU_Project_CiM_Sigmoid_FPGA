...
void init_elapsed(int n){
	elapsed = (double *)malloc(sizeof(double)*n);
...
void print_elapsed(int n){
	int i;
	// get the largest elapsed time among all threads
	double largest = 0.0;
	for(i=0; i< n; i++){
		if(elapsed[i] > largest){
			largest = elapsed[i];
		}
	}

	//print the largest elapsed time in ms (current time is in seconds)
	...
	if(public.frame_no != 0){
		...
		for(col=1; col<=public.conv_cols; col++){
			...
			for(row=1; row<=public.conv_rows; row++){
				...
				#ifdef RUN_PIM
					// allocate an array of total_array_size elements
					int *temp_array_1 = (int *)malloc(total_array_size * sizeof(int));
					int *temp_array_2 = (int *)malloc(total_array_size * sizeof(int));
					int *output_array = (int *)malloc(total_array_size * sizeof(int));
					int count = 0;
					// run the loop again and store the data into temp_array
					for(ja=ja1; ja<=ja2; ja++){
						jb = jp1 - ja;
						for(ia=ia1; ia<=ia2; ia++){
							ib = ip1 - ia;
							temp_array_1[count] = float2fix(private.d_in_mod[public.in_mod_rows*(ja-1)+ia-1]);
							temp_array_2[count] = float2fix(private.d_in2[public.in2_rows*(jb-1)+ib-1]);
							count+=1;
						}
					}
					
					// run the bbop
					bbop_op(BBOP_MUL, temp_array_1, temp_array_2, output_array, total_array_size, 1, BBOP_MUL);
					// run the reduce
					bbop_op_red(BBOP_ADD, output_array, total_array_size, 2);
		...
		for(col=1; col<=public.mask_conv_cols; col++){
			...
			int total_array_size = 0;
			...
			#ifdef RUN_PIM
				// allocate an array of total_array_size elements
				int *temp_array = (int *)malloc(total_array_size * sizeof(int));
				int count = 0;
				...
				for(ja=ja1; ja<=ja2; ja++){
					...
					for(ia=ia1; ia<=ia2; ia++){
						ib = ip1 - ia;
						temp_array[count] = float2fix(private.d_tMask[public.tMask_rows*(ja-1)+ia-1]);
						count+=1;
					}
				}
				
				// run the bbop
				bbop_op_red(BBOP_ADD, temp_array, total_array_size, 0);
